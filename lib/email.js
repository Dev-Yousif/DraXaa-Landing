...
const createTransporter = () => {
  const config = {
    ...
    // Connection timeout settings for Vercel
...
// Send lead notification email with timeout protection
export const sendLeadNotification = async (leadData) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM || `"Draxaa" <${process.env.EMAIL_USER}>`,
    ...
    if (error.message.includes("timeout")) {
      console.error("⏱️ Timeout error - SMTP server took too long to respond");
    } else if (error.code === "EAUTH") {
      console.error("🔐 Authentication error - Check EMAIL_USER and EMAIL_PASSWORD");
    } else if (error.code === "ESOCKET") {
      console.error("🔌 Connection error - Check EMAIL_HOST and EMAIL_PORT");
    }
